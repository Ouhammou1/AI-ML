...
/*   Updated: 2024/12/07 21:54:43 by bouhammo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_position_intersec  	git_first_x_intersection( t_start *var)
{
    ...
    double x_step = 0;
    double y_step = 0;
    if(is_looking_up(var->ray->ray_angle)== true)
    {
        y_inter =  ((floor(var->move->coor_y / TILE_SIZE)) * TILE_SIZE )  - 1;
        y_step = -TILE_SIZE;
    }
    if(is_looking_down(var->ray->ray_angle) == true)
    {
        // printf(" --------------------------------------  ----------   ------  \n");
        y_inter = ((floor(var->move->coor_y / TILE_SIZE) * TILE_SIZE)) + TILE_SIZE;
        y_step = TILE_SIZE;
    }
    // if( chack_angle(rad_to_deg(var->ray->ray_angle)) == 1)
    // {
    //     inter.retur = 0;
    //     return inter;
    // }
    x_inter =  ((y_inter - var->move->coor_y) / tan(var->ray->ray_angle))  + var->move->coor_x;
    x_step = y_step / tan(var->ray->ray_angle);
...
t_position_intersec git_first_y_intersection(t_start *var)
{
    ...
    if(is_looking_right(var->ray->ray_angle) == true)
    {
        printf("   ........................ OK 1  ................   \n\n");
    ...
    if( chack_angle(rad_to_deg(var->ray->ray_angle)) == 1)
        return inter;
    if (cos(var->ray->ray_angle) != 0)
    {
        printf("   ....................  \n\n");
        double cost = (sin(var->ray->ray_angle) / cos(var->ray->ray_angle));
        y_inter =  var->move->coor_y +  ((var->move->coor_x - x_inter ) * cost ) ;
        y_step = (x_step * cost);
    }
    ...
    inter.x = x_inter;
    inter.y = y_inter;
    var->ray->x_inter = inter.x;
    var->ray->y_inter = inter.y;

printf(" ----------------------->>>>>>>  y_inter [%d] \n", y_inter);
    inter.distance = sqrt(pow((x_inter - var->move->coor_x), 2) + pow((y_inter - var->move->coor_y), 2));
    var->ray->distance = inter.distance;
    return inter;
}
...
{
    var->ray->ray_angle = normalize_angle(var->draw->angle);
    ...
    //  inter_x = git_first_x_intersection(var);
        ...
        drawing_line(var, var->move->coor_x, var->move->coor_y, var->ray->x_inter , var->ray->y_inter);        
        var->ray->ray_angle += deg_to_rad( rad_to_deg(FOV_ANGLE) / 1);
		ray++;
	}
