...
/*   Updated: 2024/12/07 21:25:43 by bouhammo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void change_deriction(mlx_key_data_t keydata, t_start *var)
{
    if (keydata.action == MLX_RELEASE || keydata.action == MLX_REPEAT)
    {
        if (keydata.key == MLX_KEY_RIGHT)
        {
            var->draw->angle += deg_to_rad( ROTATION_ANGLE );
...
double position_player_y(t_start *var)
{
	int i = var->len_x -1;
	while (var->copy_map[i] != NULL)
	{
		int j = 0;
	// printf(" i = [%d] \n", i);
	// printf(" s = [%s] \n", var->copy_map[i]);
		while (var->copy_map[i][j])
		{

			if( ft_caracter(var->copy_map[i][j])  == 1) 
				return i;
			j++;
		}
		i--;
...
double position_player_x(t_start *var)
{
	int i = var->len_x -1;
	while (var->copy_map[i] != NULL)
	{
...
void	initialize_move_player(t_start *var )
{
	...
	// printf(" position_p_y = [%f] \n\n\n", position_player_y(var));
	// printf(" position_p_x = [%f] \n\n\n", position_player_x(var));
	...
	var->move->coor_x = position_player_x(var) * TILE_SIZE;
	var->move->coor_y = ( var->len_x - 1 -  position_player_y(var)) * TILE_SIZE;
