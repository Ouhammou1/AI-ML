...
/*   Updated: 2024/12/07 21:11:56 by bouhammo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// printf("map[%d] = %s\n", i, var->map[i]);
		while (var->copy_map[i][j])
		{
			...
			if (var->copy_map[i][j] == '0' || ft_caracter(var->copy_map[i][j]) == 1)
			{
				int k = y_map;
				while (k < y_map + TILE_SIZE - 1)
				{
...
void	copy_maap(t_start *var)
{
	...
	while (var->map[i])
	{
		...
		while (var->map[i][j])
		{
			var->copy_map[len][j] = var->map[i][j];
			j++;
		}
		i++;
		len--;
...
void ft_cp_map(t_start *var)
{
	int i = 0;
	int len = var->len_x - 1;
	var->copy_map = malloc(sizeof(char *) * (var->len_x + 1));
	if (!var->copy_map)
		ft_error();
	while (i < var->len_x)
	{
		var->copy_map[i] = ft_strdup(var->map[len]);
		if (!var->copy_map[i])
			ft_error();
		i++;
		len--;
	}
	var->copy_map[var->len_x] = NULL;
}
int get_r(int rgba)
{
    return ((rgba >> 24) & 0xFF);
